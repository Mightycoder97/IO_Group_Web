...
class MapController {
  ...
  setupEventListeners() {
    ...
    if (sedeInput && resultsContainer) {
      ...
      document.addEventListener("click", (event) => {
        if (
          !sedeInput.contains(event.target) &&
    ...
    document
      ...
      ?.addEventListener("click", () => this.clearMapSelection());
    document
      .getElementById("clearNearbyBtn")
      ?.addEventListener("click", () => this.clearNearbySearch());
    document
      .getElementById("clearDistrictsBtn")
      ?.addEventListener("click", () => this.clearDistricts());
    document
    ...
    document
      .getElementById("fitResultsBtn")
      ?.addEventListener("click", () => this.fitCurrentResults());
  ...
  async loadViewportSedes(options = {}) {
    ...
    if (!options.force && viewportKey && viewportKey === this.lastViewportKey)
      return;
  ...
  handleAddressSelect(place) {
    ...
    this.setReferenceLocation(
      pos,
      place.name || place.formatted_address || "Direccion",
    );
  ...
  async handleSedeSearch(query, container) {
    ...
    if (!matches.length) {
      container.innerHTML =
        '<div class="list-group-item small text-muted">No se encontraron resultados</div>';
    } else {
      ...
      container.querySelectorAll("[data-sede-id]").forEach((btn) => {
        btn.addEventListener("click", () => {
          const sede =
            this.sedeById.get(String(btn.dataset.sedeId)) ||
            matches.find(
              (item) => String(item.id_sede) === String(btn.dataset.sedeId),
            );
          if (!sede) return;
  ...
  renderSelectedSedes() {
    ...
    chipContainer.querySelectorAll("[data-sede-id]").forEach((btn) => {
      btn.addEventListener("click", () =>
        this.removeSedeFromSelection(btn.dataset.sedeId),
      );
    });
  }
  ...
  populateDistrictFilter(districtRows = []) {
    ...
    const counts = new Map(
      districtRows.map((row) => [row.distrito, Number(row.total) || 0]),
    );
    const districts = Array.from(counts.keys())
      .filter(Boolean)
      .sort((a, b) => a.localeCompare(b));
    if (!districts.length) {
      container.innerHTML =
        '<div class="text-muted small">No se encontraron distritos</div>';
      return;
    }
  ...
  clearDistricts(options = {}) {
    ...
    document.querySelectorAll(".filter-district").forEach((cb) => {
      cb.checked = false;
    });
    this.updateDistrictSummary();
  ...
  scheduleApplyFilters(options = {}) {
    this.pendingFilterOptions = {
      forceFit: Boolean(
        this.pendingFilterOptions?.forceFit || options.forceFit,
      ),
    };
  ...
  applyFilters(options = {}) {
    ...
    const sourceSet = this.selectedSedes.size
      ? Array.from(this.selectedSedes)
          .map((id) => this.sedeById.get(String(id)))
          .filter(Boolean)
      : this.sedes;
    const center = this.getReferenceCenter();
    const keepNoFrequency =
      this.filters.frequencies.size === this.allFrequencies.length;
    ...
    for (const sede of sourceSet) {
      ...
      if (hasDistrictFilter && !this.filters.districts.has(sede._district))
        continue;
      ...
      if (this.filters.nearby) {
        ...
        if (
          Math.abs(position.lat - center.lat) > latLimit ||
          Math.abs(position.lng - center.lng) > lngLimit
        ) {
          continue;
        }
    ...
    if (this.filters.nearby) {
      filtered.sort(
        (a, b) => (a._distanceKm ?? 9999) - (b._distanceKm ?? 9999),
      );
    } else {
      filtered.sort((a, b) =>
  ...
  getOrCreateSedeMarker(sede) {
    ...
    const color = this.isActive(sede)
      ? this.frequencyColors[sede._frequency] || "#198754"
      : "#6c757d";
  ...
  renderNearbyList(sedes) {
    ...
    if (!sedes.length) {
      list.innerHTML =
        '<div class="nearby-empty">Sin sedes activas en este radio.</div>';
      return;
    }

    list.innerHTML = sedes
      .slice(0, 10)
      .map(
        (sede) => `
  ...
  showInfoWindow(marker, sede) {
    const distance =
      sede._distanceKm !== null && sede._distanceKm !== undefined
  ...
  setReferenceLocation(pos, title = "Referencia") {
    ...
    if (!this.referenceMarker) {
      this.referenceMarker = this.createUtilityMarker(
        pos,
        title,
        "#dc3545",
        true,
  ...
  updateDistrictSummary() {
    ...
    if (districtSummary)
      districtSummary.textContent = count ? `${count} distritos` : "Todos";
  }
  ...
  updateCounters(sedes) {
    ...
    const totalLabel = this.mapMeta.total
      ? `${sedes.length}/${this.mapMeta.total}`
      : sedes.length;
    ...
    if (active) {
      ...
      active.textContent = this.mapMeta.active
        ? `${activeCount}/${this.mapMeta.active}`
        : activeCount;
    }
    if (mode)
      mode.textContent = this.filters.nearby
        ? "Cercanas activas"
        : "Vista actual";
  }
  ...
  isAllowedMapPosition(position) {
    return Object.values(this.allowedRegions).some(
      (bounds) =>
  ...
  updateUserMarker(pos) {
    if (!this.userMarker) {
      this.userMarker = this.createUtilityMarker(
        pos,
        "Mi ubicacion",
        "#0d6efd",
      );
