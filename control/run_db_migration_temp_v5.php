<?php
/**
 * IO Group - Script Temporal v5 de Ejecución de Migración de Servicios 2026
 * 
 * Este script debe ser accedido via HTTP con un token de seguridad:
 * https://iogroup.pe/control/run_db_migration_temp_v5.php?token=io_group_migration_2026_secret_9988
 */
...
// Búsqueda recursiva de archivos .env
$found_env_files = [];
function find_env_files($dir, &$results) {
...
find_env_files("/home/u511863531/domains/iogroup.pe", $found_env_files);

// Filtrar las claves de $_SERVER para ver si hay variables de entorno interesantes
$server_keys = [];
foreach ($_SERVER as $key => $value) {
    $lower = strtolower($key);
    if (strpos($lower, 'db') !== false || strpos($lower, 'user') !== false || strpos($lower, 'pass') !== false || strpos($lower, 'port') !== false || strpos($lower, 'env') !== false) {
        // Ocultar contraseñas reales por seguridad si las hubiera
        if (strpos($lower, 'pass') !== false || strpos($lower, 'secret') !== false) {
            $server_keys[$key] = "***hidden***";
        } else {
